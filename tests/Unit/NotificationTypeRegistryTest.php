...
class NotificationTypeRegistryTest extends TestCase
{
    ...
    public function test_register_and_all(): void
    {
        ...
        $this->assertContains(NotificationType::FORGOT_PASSWORD, $values);
        $this->assertContains(NotificationType::CUSTOM, $values);
    ...
    public function test_metadata_label_and_icon(): void
    {
        ...
        $ntModel = new NotificationType();
        $this->assertEquals('Meta Label', $ntModel->label('meta_type'));
        $this->assertEquals('lucide:meta', $ntModel->icon('meta_type'));
    }
}
