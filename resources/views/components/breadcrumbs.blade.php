    ...
    <div class="flex items-center gap-2">
        @if($icon)
            @if($backUrl)
                <a
                    href="{{ $backUrl }}"
                    class="inline-flex items-center justify-center w-9 h-9 rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 text-gray-500 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-gray-700 hover:text-gray-700 dark:hover:text-gray-200 transition-colors"
                    title="{{ __('Go back') }}"
                >
                    <iconify-icon icon="{{ $icon }}" width="20" height="20"></iconify-icon>
                </a>
            @else
                <span class="inline-flex items-center justify-center w-9 h-9 rounded-lg bg-gray-100 dark:bg-gray-800 text-gray-500 dark:text-gray-400">
                    <iconify-icon icon="{{ $icon }}" width="20" height="20"></iconify-icon>
                </span>
            @endif
            <iconify-icon icon="lucide:chevron-right" width="16" height="16" class="text-gray-400 dark:text-gray-500"></iconify-icon>
        @elseif($backUrl)
            <a
                href="{{ $backUrl }}"
                class="inline-flex items-center justify-center w-9 h-9 rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 text-gray-500 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-gray-700 hover:text-gray-700 dark:hover:text-gray-200 transition-colors"
