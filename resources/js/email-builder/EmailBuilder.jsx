...
const EmailBuilder = ({ initialData, onSave, onImageUpload, onVideoUpload, templateData, listUrl }) => {
    const [blocks, setBlocks] = useState(initialData?.blocks || []);
    const [canvasSettings, setCanvasSettings] = useState(initialData?.canvasSettings || defaultCanvasSettings);
    ...
    const initialStateRef = useRef({
        ...
        canvasSettings: JSON.stringify(initialData?.canvasSettings || defaultCanvasSettings),
    ...
    useEffect(() => {
        ...
        const hasChanges =
            ...
            currentState.canvasSettings !== initialStateRef.current.canvasSettings ||
            currentState.templateName !== initialStateRef.current.templateName ||
            currentState.templateSubject !== initialStateRef.current.templateSubject;
        ...
        const handleKeyDown = (e) => {
                ...
                if (isNested && parentId !== null && columnIndex !== null) {
                    ...
                    setBlocks(prev => prev.map(block => {
                        if (block.id === parentId) {
                            ...
                            newChildren[columnIndex] = newChildren[columnIndex].filter(b => b.id !== selectedBlockId);
                            return { ...block, props: { ...block.props, children: newChildren } };
                        }
                        return block;
                    }));
                } else {
            ...
            if (e.key === 'Enter') {
                ...
                const newBlock = {
                    id: uuidv4(),
                    type: 'text',
                    props: { ...textBlockConfig.defaultProps, content: '<p>New text block...</p>' },
                };
            ...
            if (active.id !== over.id && overData?.type === 'nested') {
                ...
                if (sourceParentId === targetParentId && sourceColumnIndex === targetColumnIndex) {
                    setBlocks(prev => prev.map(block => {
                        if (block.id === sourceParentId) {
                            const newChildren = [...(block.props.children || [])];
                            const column = [...(newChildren[sourceColumnIndex] || [])];
                            const oldIndex = column.findIndex(b => b.id === active.id);
                            const newIndex = column.findIndex(b => b.id === over.id);
                            if (oldIndex !== -1 && newIndex !== -1) {
                                newChildren[sourceColumnIndex] = arrayMove(column, oldIndex, newIndex);
                            }
                            return { ...block, props: { ...block.props, children: newChildren } };
                        }
                        return block;
                ...
                setBlocks(prev => {
                    ...
                    const afterRemove = prev.map(block => {
                        if (block.id === sourceParentId) {
                            const newChildren = [...(block.props.children || [])];
                            const sourceColumn = [...(newChildren[sourceColumnIndex] || [])];
                            const blockIndex = sourceColumn.findIndex(b => b.id === active.id);
                            if (blockIndex !== -1) {
                    ...
                    return afterRemove.map(block => {
                        if (block.id === targetParentId) {
                            ...
                            const insertIndex = targetColumn.findIndex(b => b.id === over.id);
                            if (insertIndex !== -1) {
                ...
                setBlocks(prev => {
                    ...
                    return afterRemove.map(block => {
                        if (block.id === targetParentId) {
                            ...
                            newChildren[targetColumnIndex] = [...newChildren[targetColumnIndex], movedBlock];
                            return { ...block, props: { ...block.props, children: newChildren } };
                        }
                        return block;
                    });
                ...
                if (block.type === 'columns' && block.props.children) {
                    const newChildren = block.props.children.map(column =>
                        column.map(nestedBlock =>
                            nestedBlock.id === blockId ? { ...nestedBlock, props: newProps } : nestedBlock
                        )
                    );
                    return { ...block, props: { ...block.props, children: newChildren } };
                }
    ...
    const handleDeleteNestedBlock = useCallback((blockId, parentId, columnIndex) => {
        setBlocks(prev => prev.map(block => {
            if (block.id === parentId) {
                const newChildren = [...(block.props.children || [])];
                if (newChildren[columnIndex]) {
                    newChildren[columnIndex] = newChildren[columnIndex].filter(b => b.id !== blockId);
                }
                return { ...block, props: { ...block.props, children: newChildren } };
            }
            return block;
    ...
    const handleDuplicateBlock = useCallback((blockId) => {
        setBlocks(prev => {
            ...
            if (blockToDuplicate.type === 'columns' && blockToDuplicate.props.children) {
                duplicatedBlock.props.children = blockToDuplicate.props.children.map(column =>
                    column.map(nestedBlock => ({
                        ...nestedBlock,
                        id: uuidv4(),
    ...
    const handleMoveNestedBlock = useCallback((blockId, parentId, columnIndex, direction) => {
        setBlocks(prev => prev.map(block => {
            if (block.id === parentId) {
                const newChildren = [...(block.props.children || [])];
                const column = [...(newChildren[columnIndex] || [])];
                ...
                const newIndex = direction === 'up' ? index - 1 : index + 1;
                if (newIndex < 0 || newIndex >= column.length) return block;
    ...
    const handleDuplicateNestedBlock = useCallback((blockId, parentId, columnIndex) => {
        setBlocks(prev => prev.map(block => {
            if (block.id === parentId) {
                const newChildren = [...(block.props.children || [])];
                const column = [...(newChildren[columnIndex] || [])];
    ...
    const handleSave = async () => {
        if (!templateName.trim()) {
            setSaveStatus({ type: 'error', message: 'Template name is required' });
            return;
        }
    ...
    const customCollisionDetection = useCallback((args) => {
        ...
        if (pointerCollisions.length > 0) {
            ...
            const nestedCollision = pointerCollisions.find(c => c.data?.droppableContainer?.data?.current?.type === 'nested');
            if (nestedCollision) {
                return [nestedCollision];
            }

            // Then check for column collisions
            const columnCollision = pointerCollisions.find(c => c.id.toString().startsWith('column-'));
            if (columnCollision) {
            ...
            <div className="h-screen flex flex-col bg-gray-100">
                ...
                <header className="bg-white border-b border-gray-200 px-4 py-3 flex items-center justify-between shadow-sm flex-shrink-0">
                    <div className="flex items-center gap-4">
                        ...
                        <h1 className="text-lg font-semibold text-gray-800">
                            {templateData?.uuid ? 'Edit Template' : 'Create Template'}
                        </h1>
                        {isDirty && (
                ...
                <div className="flex-1 flex overflow-hidden">
                    ...
                    <div className="w-64 bg-white border-r border-gray-200 p-4 overflow-hidden flex flex-col flex-shrink-0">
                        <h3 className="text-sm font-semibold text-gray-900 mb-4">Blocks</h3>
