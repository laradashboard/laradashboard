...
export default createBlock({
    ...
    fields: [
        ...
        { name: 'height', type: 'select', label: 'Thickness', section: 'Style', options: [
            { value: '1px', label: 'Thin (1px)' },
            { value: '2px', label: 'Medium (2px)' },
            { value: '4px', label: 'Thick (4px)' },
        ]},
        { name: 'margin', type: 'select', label: 'Spacing', section: 'Layout', options: [
    ...
    save: {
        page: (props) => `<hr style="border:none; height:${props.height}; background:${props.color}; margin:${props.margin} 0;" />`,
        email: (props) => `<table width="100%" cellpadding="0" cellspacing="0"><tr><td style="padding:${props.margin} 0;"><div style="height:${props.height}; background:${props.color};"></div></td></tr></table>`,
    },
});
```
...
```javascript
import { emailTable, emailButton, emailSpacer, emailTextStyles } from '@lara-builder/factory';
...
export default function DealCardBlock({ props, isSelected }) {
    ...
    return (
        <div style={containerStyle}>
            ...
            <div style={{ fontSize: '24px', fontWeight: '700', color: props.accentColor }}>
                {props.value}
            </div>
...
export const page = (props) => {
    ...
    const styles = mergeBlockStyles(props, `
...
├── blocks/                  # Core blocks
│   └── spacer/              # Example simplified block
│       ├── index.js         # 3 lines using factory
│       ├── block.json       # Metadata + fields
│       ├── block.jsx        # Canvas component
...
export default function SpacerBlock({ props, isSelected }) {
    return (
        <div style={{
            ...
            border: isSelected ? '1px dashed #9ca3af' : '1px dashed transparent',
            display: 'flex',
            alignItems: 'center',
            justifyContent: 'center'
        }}>
            {isSelected && <span style={{ color: '#9ca3af', fontSize: '12px' }}>{props.height}</span>}
        </div>
    );
}
```
